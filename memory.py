...
class MemoryDNN:
    def __init__(
        ...
        self.memory = np.zeros((self.memory_size, self.net[0] + self.net[-1] + 1))
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('train'):
            self._train_op = tf.train.AdamOptimizer(learning_rate=self.lr, beta1=0.9, epsilon=1e-3).minimize(self.loss)
            # self._train_op = tf.train.AdamOptimizer(self.lr, 0.09).minimize(self.loss)

    def remember(self, h, E_i, D_i, w, m):
        # replace the old memory with new memory
        idx = self.memory_counter % self.memory_size
        #for i in index_list:
        #    m = np.insert(m, i, 0)
        # self.memory[idx, :] = np.hstack((h, m, D_i, E_i, w))
        self.memory[idx, :] = np.hstack((h, D_i, E_i, m, w))
        self.memory_counter += 1

    def encode(self, h, E_i, D_i, w, m):
        # encoding the entry
        self.remember(h, E_i, D_i, w, m)
    ...
    def learn(self):
        ...
        m_train = batch_memory[:, self.net[0]:self.net[0] + self.net[-1]]
    ...
    def decode(self, h, E_i, D_i, index_list, k=1, mode='OP'):
        # to have batch dimension when feed into tf placeholder
        temp_h = np.append(h, D_i)
        temp_h = np.append(temp_h, E_i)
        temp_h = temp_h[np.newaxis, :]

        m_pred = self.sess.run(self.m_pred, feed_dict={self.h: temp_h})
        m_pred = np.delete(m_pred, index_list, 1)
        # temp_k=[]
        if mode is 'OP':
    ...
    def knm(self, m, k=1):
        ...
        if k > 1:
            ...
            for i in range(k):
                if i >= len(idx_list) or idx_list[i] >= len(m):
                    continue
                if m[idx_list[i]] > 0:
