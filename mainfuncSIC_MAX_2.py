...
def EAOO_latest(N_, n_, E_min_, P_, E_i_, D_i_list_, f_i_, g_i_, B_=5, T_=2.0):
    ...
    return total_time, totallantency_final, stop_time, latency_res_all
...
if __name__ == "__main__":
    ...
    print("EAOOsic_lantency_average of each 10-30 devices,", EAOOSIC_latency_list)

    for i in range(len(All_latency_list)):
        latency_res_all_average = All_latency_list[i]
        EAOOSIC_latency_average = EAOOSIC_latency_list[i]
        print("第 %d 轮：遍历latency和 / EAOO latency和 = %.4f / %.4f = %.4f" %
              (i + 1, latency_res_all_average, EAOOSIC_latency_average,
               latency_res_all_average / EAOOSIC_latency_average))