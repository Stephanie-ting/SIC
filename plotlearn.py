...
def plot_cost_EAOOSIC(learn, color):
    ...
    for i in range(len(learn)):
        ...
        EAOOSIC_cost = sio.loadmat('./learn/cost_EAOOSIC_%g' % learn[i])['learn_cost_list']
        EAOOSIC_cost = EAOOSIC_cost[0, :].tolist()
        plt.plot(np.arange(len(EAOOSIC_cost)) * 10, EAOOSIC_cost, color=color[i], label= 'Learning rate = '+ str(learn[i]))
...
if __name__ == '__main__':
    learn = [0.0001,0.000015, 0.00001, 0.0000095]
