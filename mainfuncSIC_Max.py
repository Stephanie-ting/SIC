...
def EAOO_latest(N_, n_, E_min_, P_, E_i_, D_i_list_, f_i_, g_i_, B_=5, T_=2, Ps_=50):
    ...
    for i in range(n):
        ...
        # *对上传的设备进行分组
        def split_group_lantency(m, mode="first"):
            current_up = []  # 存的下标
            current_local = []  # 存的下标
            current_single = []  # 存的下标
            current_par = []  # 存的下标
            ...
            if mode == "first":
                ...
                for device in current_local:
                    up_local_lantency += D_i[device] * g_i[device] / f_i[device]                
        ...
        for m in m_list:
            ...
            if time_limit <= T:
                ...
                for i in range(len(m)):
                    ...
                    #all_lantency_list.append(m_lantency)
        ...
        lantency_all_min = 99999999999
        final_m_best = []
        #* 计算 m_list_all 中的最佳情况
        for m in m_list_all:
            single_lantency, maxPar_lantency, up_local_lantency = split_group_lantency(m.copy(), "second")
            time_limit = sum(maxPar_lantency) + single_lantency       
            ...
            if time_limit <= T:
                ...
                for i in range(len(m)):
                    ...
                    lantency_all_temp = time_limit + up_local_lantency
                    #* 遍历得到最小的一个
                    if lantency_all_temp < lantency_all_min:
                        final_m_best = m
                        lantency_all_min = lantency_all_temp

        accuracy_temp = lantency_all_min / totallantency_singleframe
        print("\n\n第 %d 次循环的结果为 %.4f" % (counter, accuracy_temp))
        print("final_m_best: ", final_m_best)
        print("final_m     : ", final_m)
        if accuracy_temp > 1.0001:
            print("第 %d 次循环 出现错误！！！！此时为:%.4f" % (counter, accuracy_temp))
            input()
        counter += 1
