...
def getfeasibleres(edge_list, upload, recordD_i, recordg_i, recordf_i, E_min, C_i_l, C_i_e, E_i,wireless_devices_,server_,alpha_,N_0_,beta_,T_):
    WDlist = edge_list[:]
    E_mintemp = E_min[:]
    C_ltemp = C_i_l[:]
    C_etemp = C_i_e[:]
    ...
    for device in edge_devices:
        ...
        recordpower.append(device.power / distance_to ** alpha)
    ...
    orderpower = []   #记录各设备排序后的p/d**alpha值
    ...
    while num > 0:
        ...
        minindex = recordpower.index(max(recordpower))  #选取的这个p/d**alpha最小的设备下标
