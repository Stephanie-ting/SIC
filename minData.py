...
if __name__ == '__main__':
    ...
    for lowdata in range(50, 160, 10):
        ...
        # 任务数据量 均匀分布 100-150 [N*n]
        D_i_list = np.mat(abs(np.random.uniform(lowdata, high=150, size=n * N)).reshape(n, N))
    ...
    save_to_txt(EAOOSIC_minData_latency_list, 'EAOOSIC_minData_latency_list.txt')
    save_to_txt(EAOO_minData_latency_list, 'EAOO_minData_latency_list.txt')
    save_to_txt(DROO_minData_latency_list, 'DROO_minData_latency_list.txt')
    save_to_txt(local_minData_latency_list, 'local_minData_latency_list.txt')
# coding=utf8
