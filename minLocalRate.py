...
if __name__ == '__main__':
    EAOOSIC_localrate_latency_list = []
    EAOO_localrate_latency_list = []
    DROO_localrate_latency_list = []
    local_localrate_latency_list = []
    B_ = 30
    T_ = 1
    #Ps_ = 50                               #20 40 60 80 100 110 120 140 160 180 200
    for lowrate in range(20, 220, 20):  # 20 60 100 140 180 220 260 300 340 380
        N = 20
        n = 5000
        ...
        EAOOSIC_localrate_latency_list.append(EAOOSIC_lantency_average)

        # EAOO-串行
        EAOO_time, EAOO_lantency, stop_time_ori = EAOO_latest_serial(N, n, E_min, P, E_i, D_i_list, f_i, g_i, B_, T_)
        EAOO_lantency_average = EAOO_lantency / (stop_time_ori + 1)
        EAOO_localrate_latency_list.append(EAOO_lantency_average)

        # DROO算法
        DROO_time, DROO_lantency, stop_time_droo, disrunnable_times = DROO_latest_serial(N, n, E_min, P, E_i, D_i_list, f_i, g_i, B_, T_)
        DROO_lantency_average = DROO_lantency / (stop_time_droo + 1 - disrunnable_times)  # * 获得具体停止的时间帧stop_time，根据改时间帧得到平均lantency
        DROO_localrate_latency_list.append(DROO_lantency_average)
        # 完全本地
        ...
        local_localrate_latency_list.append(local_lantency_average)

    save_to_txt(EAOOSIC_localrate_latency_list, 'EAOOSIC_localrate_latency_list.txt')
    save_to_txt(EAOO_localrate_latency_list, 'EAOO_localrate_latency_list.txt')
    save_to_txt(DROO_localrate_latency_list, 'DROO_localrate_latency_list.txt')
    save_to_txt(local_localrate_latency_list, 'local_localrate_latency_list.txt')
# coding=utf8
